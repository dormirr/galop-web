...
const BasicForm = () => {
  ...
  const onChange = info => {
    if (info.file.status === 'done') {
      message.success(`文件上传成功！共注销${info.file.response.suc}个账号！`);
    } else if (info.file.status === 'error') {
      message.error(
        `文件上传失败！共注销${info.file.response.suc}个账号，失败${info.file.response.err}个账号！`,
      );
    }
  };

  return (
    <PageHeaderWrapper content="通过 Excel 批量注销用户账号。">
      <Card bordered={false}>
        <Form
          style={{
            marginTop: 8,
